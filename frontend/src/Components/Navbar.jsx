      ...
      <div className="container mx-auto px-4 py-3 flex justify-between ">
        ...
        <nav className="flex flex-wrap items-center gap-5 ">
          ...
          <Link to={"/cart"} className="relative group "><ShoppingCart className=" inline-block mr-1 group-hover:text-pupkin_spice text-gray-200" size={20} />
            ...
            {cart?.userProducts.length > 0 && <span className="absolute -top-3 -left-3 bg-orange-700 text-white text-xs  rounded-full px-2 py-0.5 group-hover:bg-pupkin_spice transition-all ease-in-out duration-300">
          ...
          {authUser ? (
            <>
              ...
              <button onClick={handleLogout}
                className="text-gray-200 hover:bg-pupkin_spice transition-all ease-in-out duration-300 flex items-center bg-yellow-900 px-4 py-2 rounded-md  ">
                {/* <Link to={"/"} > */}
                <LogInIcon className="mr-2  rotate-180 size-5" />
                <span className=" ">
                  Logout
                </span>
                {/* </Link> */}
              ...
              <Link to={"/account"} className="relative group"><User2Icon className=" inline-block mr-1 group-hover:text-pupkin_spice text-gray-200" size={20} />
                <span className="hidden sm:inline text-gray-200 hover:text-pupkin_spice transition-all ease-in-out duration-300 rounded-full bg-pumpkinDark hover:bg-taupeDark p-3">{getInitials(authUser?.user?.userName || " ")}</span>
              </Link>
